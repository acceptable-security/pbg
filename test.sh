...
for i in $(seq 1 $TEST_ITERS); do
	...
	for test in tests/*; do
		...
		TIMEFORMAT='time=%U mem=%k RSS=%M'

		create_time=$(command time -f $TIMEFORMAT ( $PBG project create -db=$db -backend=$BACKEND -config=$conf 2>&3 1>&3 0>&3 ) 3>$OUTPUTDIR/$name.create.$i.txt)
		...
		query_time=$(command time -f $TIMEFORMAT ( $PBG project query -db=$db -backend=$BACKEND -datalog=$OUTPUTDIR/datalog_dir/ 2>&3 1>&3 0>&3 ) 3>$OUTPUTDIR/$name.query.$i.txt)

		echo "Running memory tests"
		test_time=$(command time -f $TIMEFORMAT ( $SOUFFLE --fact-dir=$OUTPUTDIR/datalog_dir -c -j64 ./queries/mem-test.dl  2>&3 1>&3 0>&3) 3>$OUTPUTDIR/$name.test.$i.txt)
